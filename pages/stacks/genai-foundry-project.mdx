...
<Warning>
**The parent GenAI Foundry stack deploy and destroy can take up to 180 minutes** when using private networking. This is due to Azure's internal VNet injection provisioning (managed Container Apps environment, network peering, and DNS forwarding). The project stack itself deploys in minutes, but the parent Foundry account must complete VNet injection and capability host creation first. Ensure the parent stack is fully deployed before running this stack.
</Warning>
